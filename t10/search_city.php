 ...
 if (isset($_GET['page'])) {
     ...
     $page = 1;
      ...
      if (empty($filtered_disabled)) {
          ...
          $query3 =   "select isPublic, toilet_name, road_addr, isUnisex, m_d_toilet, w_d_toilet, open_time from toilet where regexp_like(road_addr, '{$filtered_gu}') && (m_d_toilet != 0 || w_d_toilet != 0) limit {$start_num}, {$list}";
          $result3 = mysqli_query($link, $query3) or die(mysqli_error($link));
          $gu_info = '';
          while ($row = mysqli_fetch_array($result3)) {
              $gu_info .= '<tr>';
