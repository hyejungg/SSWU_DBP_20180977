...
$link = mysqli_connect('localhost:3307', 'root', 'root06', 'dbp');
 ...
 if (isset($_GET['page'])) {
     ...
     $page = 1;
      ...
      $query2 = "select isPublic, toilet_name, road_addr, isUnisex, m_d_toilet, w_d_toilet, open_time from toilet limit {$start_num}, {$list}";
      ...
      if (isset($_GET['city']) && isset($_GET['gu'])) {
          ...
          $query3 =   "select isPublic, toilet_name, road_addr, isUnisex, m_d_toilet, w_d_toilet, open_time from toilet where regexp_like(road_addr, '{$filtered_gu}') limit {$start_num}, {$list}";
          ...
          $filtered_id = mysqli_real_escape_string($link, $_GET['city']);
          // $query3 = "select isPublic, toilet_name, road_addr, isUnisex, m_d_toilet, m_d_urinal, w_d_toilet, open_time from toilet WHERE  limit {$start_num}, {$list}";
          $query3 = "select isPublic, toilet_name, road_addr, isUnisex, m_d_toilet, w_d_toilet, open_time from toilet where regexp_like(road_addr, {$filtered_id}) limit {$start_num}, {$list}";
